...
public class Document 
{
	...
	public String receiveId()
	{
		...
		for(int i = 0; i < windowHeap.size(); ++i)
		{
			...
			{
				return ((StdIdentifierWindow)temp).getId();
			}
		}
	...
	public TestWorkData getTestWorkData(Mat testSheet)
	{
		double colorDivider = getColorDivider(testSheet);
		
		updateAllQuestions(testSheet, colorDivider);
		
		Mark totalMark = getTotalMark();
		
		int testWorkID = 1;
		
		updateIdentifier(testSheet, colorDivider);
		
		String choosenStudentID = receiveId();
		
		ImageIO.DisplayVerticalMat(testSheet);
		
		return new TestWorkData(choosenStudentID, testWorkID, totalMark);
	}
}
