...
public final class ImageProcessing 
{
	...
	public static void drawGrid(Mat img, int num)
	{
		for(int i = 1; i < num; ++i)
		{
			...
			Imgproc.putText(img, Double.toString(((double)i / num)), new Point(0, ((double)i / num) * (img.rows() - 1)), 1, 0.8, new Scalar(0));
			Imgproc.putText(img, Double.toString(((double)i / num)),
					new Point(((double)i / num) * (img.cols()), ((double)0.5 / num) * (img.cols())), 1, 0.8, new Scalar(0));
		}
	}
